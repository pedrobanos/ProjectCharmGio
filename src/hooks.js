  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 🔹 Reembolsos aplicados al mes actual
        const reembolsosMesActual = reembolsos.filter((r) => r.mes_aplicado === `${year}-${String(selectedMonth + 1).padStart(2, "0")}`);
        ...
        const pendienteAnterior = await getPendienteMesAnterior("carol", year, selectedMonth, listSales);
        setSaldoMesAnterior(pendienteAnterior);

        // 🔹 Saldo mes actual = ventas - reembolsos aplicados a este mes
        const saldoMesActual = Math.max(totalCarolMes - totalReembolsadoMes, 0);
        setSaldoMes(saldoMesActual);

        // 🔹 Saldo total pendiente = mes anterior (si queda algo) + mes actual
