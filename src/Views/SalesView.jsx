...
const SalesView = ({ productId, userRole }) => {
  ...
  const {
    ...
    refetch, // 👈 ahora se puede recargar desde el backend
  } = useFetchSales({ year, selectedMonth, productId });

  const { loading: loadingLowStock, lowStockProducts } =
  ...
  // 🔹 Nuevo cálculo visual (mes + pendiente anterior)
  const pendienteAnterior = Math.max(saldoTotalPendiente - saldoMes, 0);
  const saldoMostradoCarol = Math.max(saldoMes + pendienteAnterior, 0);
      ...
      <div className="flex flex-row items-center gap-4 w-full mb-4">
        ...
        {filterBy === "carol" && saldoTotalPendiente > 0 && !showLowStock && (
          <button
            ...
            className="px-4 py-2 bg-green-600 text-white rounded-lg cursor-pointer font-semibold hover:bg-green-700 transition"
          >
            {userRole === "admin" ? "Iniciar reembolso" : "Ver Reembolso"}
          </button>
        )}
        ...
        <ReembolsoModal
          ...
          saldoInicial={saldoTotalPendiente}
          onClose={() => setIsReembolsoOpen(false)}
          onReembolso={async (importe) => {
            // 🧮 Lógica: restar primero del pendiente anterior, luego del mes actual
            setSaldoTotalPendiente((prevTotal) => {
              const pendienteAnterior = Math.max(prevTotal - saldoMes, 0);
              let restante = importe;
              let nuevoSaldoMes = saldoMes;
              let nuevoTotal = prevTotal;

              if (pendienteAnterior > 0) {
                const usado = Math.min(pendienteAnterior, restante);
                nuevoTotal -= usado;
                restante -= usado;
              }

              if (restante > 0) {
                const usadoMes = Math.min(saldoMes, restante);
                nuevoSaldoMes -= usadoMes;
                nuevoTotal -= usadoMes;
              }

              setSaldoMes(Math.max(nuevoSaldoMes, 0));
              return Math.max(nuevoTotal, 0);
            });
        ...
        {!showLowStock ? (
          <table className="table-fixed w-full border-collapse text-sm sm:text-base">
              ...
              {filteredSales.map((s) => {
                ...
                return (
                  <tr key={s.id} className="text-center hover:bg-gray-50">
                    ...
                    <td className="px-2 py-2 text-green-600 font-bold">
                      {s.cantidad}
                    </td>
                    <td className="px-2 py-2">
                      {s.precio_venta != null ? `${s.precio_venta} €` : "-"}
            ...
            <tfoot className="bg-gray-50 font-bold text-gray-800 sticky bottom-0">
              ...
              {filteredSales.some(
                (s) => !(s.clienteNombre || "").toLowerCase().includes("carol")
              ) && (
                  <tr className="border-t border-gray-300">
                    <td colSpan={5} className="px-4 py-2 text-start">
                      Beneficio:
                ...
                (s) => (s.clienteNombre || "").toLowerCase().includes("carol")
                  ...
                  <>
                    <tr className="border-t border-gray-300">
                      ...
                      <td
                        colSpan={2}
                        className="px-4 py-2 text-center text-blue-600"
                      >
                        {saldoMostradoCarol.toFixed(2)} €
                      </td>
                    </tr>
                    <tr className="border-t border-gray-300">
                      <td colSpan={5} className="px-4 py-2 text-start">
            ...
            <tbody className="divide-y divide-gray-200 text-gray-800">
              {lowStockProducts?.data?.map((p) => (
                <tr key={p.id} className="text-center hover:bg-gray-50">
                  ...
                  <td className="px-2 py-2">{p.nombre}</td>
                  <td className="px-2 py-2 text-red-600 font-bold">
                    {p.cantidad}
                  </td>
                  <td className="px-2 py-2">{p.total_vendido}</td>
                  <td className="px-2 py-2">{p.lugar || "-"}</td>
