...
//                 lugarLower: p.lugar?.toLowerCase() || "",
//                 codigoProveedorLower: p.codigoProveedor?.toLowerCase() || "",
//             }));
...
//                 p_precio_venta: Number(precioVenta) || null,
//             });
...
const ProductsPage = () => {
    ...
    useEffect(() => {
        if (!precioTotalLote || batchProducts.length === 0) return;
        ...
        setBatchProducts((prev) =>
            prev.map((p) => ({ ...p, precioVenta: precioUnitario.toFixed(2) }))
        );
        ...
        try {
            ...
            for (const p of batchProducts) {
                const { data: sale, error } = await supabase.rpc("confirm_sale", {
                    ...
                    p_dia: null,
                    p_cliente: cliente || null,
                    p_precio_venta: Number(p.precioVenta) || null,
                });
                if (error) throw new Error(error.message);
                setSales((prev) => [sale, ...(prev || [])]);
            }
